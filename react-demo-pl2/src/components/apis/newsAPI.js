...
// Serviço de proxy gratuito para converter RSS para JSON
...
// Carrega e devolve as notícias para usar no componente React
export async function carregarNoticias(forcarRefresh = false, noticiasAtuais = []) {
    try {
        if (forcarRefresh || noticiasAtuais.length === 0) {
            return await carregarMultiplasFontes();
        }
        return noticiasAtuais;
    } catch (error) {
        console.error('Erro ao carregar notícias:', error);
        return getNoticiasMock();
    }
}